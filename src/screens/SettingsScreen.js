...
export default function SettingsScreen() {
  ...
  const carregarDados = async () => {
    try {
      ...
      const diary = JSON.parse(await AsyncStorage.getItem('@serenar_diary') || '[]');
      const panic = JSON.parse(await AsyncStorage.getItem('@serenar_panic') || '[]');
    ...
    <LinearGradient colors={['#0a0e27', '#1a1040']} style={{flex:1}}>
      <SafeAreaView style={{flex:1, paddingTop: insets.top + 8}}>
        <ScrollView contentContainerStyle={{padding:20}}>
          <Text style={{color:'#fff', fontSize:22, fontWeight:'bold', marginBottom:4}}>âš™ï¸ Ajustes</Text>
          ...
          <View style={{backgroundColor:'rgba(255,255,255,0.06)', borderRadius:16, padding:16, marginBottom:16}}>
            <Text style={{color:'#fff', fontSize:16, fontWeight:'600', marginBottom:12}}>ðŸ‘¤ Dados Pessoais</Text>
            ...
            <Campo label="Plano de saÃºde" value={userData.planoSaude} onChange={v => atualizar('planoSaude', v)} placeholder="Qual plano?" />
          </View>

          {/* Profissionais */}
          <View style={{backgroundColor:'rgba(255,255,255,0.06)', borderRadius:16, padding:16, marginBottom:16}}>
            <Text style={{color:'#fff', fontSize:16, fontWeight:'600', marginBottom:12}}>ðŸ§‘â€âš•ï¸ Profissionais</Text>
            ...
            {userData.psicologoTel ? (
              <TouchableOpacity onPress={() => ligarProfissional(userData.psicologoNome || 'Psicólogo', userData.psicologoTel)} style={{marginTop:6}}>
                <Text style={{color:'#22c55e', fontSize:13}}>ðŸ“± Chamar no WhatsApp</Text>
              </TouchableOpacity>
            ) : null}
          ...
          {/* Emergência */}
          <View style={{backgroundColor:'rgba(255,255,255,0.06)', borderRadius:16, padding:16, marginBottom:16}}>
            <Text style={{color:'#fff', fontSize:16, fontWeight:'600', marginBottom:12}}>ðŸš¨ Contato de Emergência</Text>
            ...
            {userData.emergenciaTel ? (
              <TouchableOpacity onPress={ligarEmergencia} style={{backgroundColor:'#ef4444', paddingVertical:12, borderRadius:30, alignItems:'center', marginTop:10}}>
                <Text style={{color:'#fff', fontSize:15, fontWeight:'600'}}>ðŸ“ž Ligar agora</Text>
              </TouchableOpacity>
            ) : null}
          </View>
          ...
          <TouchableOpacity onPress={salvarDados} style={{backgroundColor:'#7c4dff', paddingVertical:14, borderRadius:30, alignItems:'center', marginBottom:16}}>
            <Text style={{color:'#fff', fontSize:16, fontWeight:'600'}}>{salvo ? 'âœ… Salvo!' : 'ðŸ’¾ Salvar dados'}</Text>
          </TouchableOpacity>

          {/* Estatísticas */}
          <View style={{backgroundColor:'rgba(255,255,255,0.06)', borderRadius:16, padding:16, marginBottom:16}}>
            <Text style={{color:'#fff', fontSize:16, fontWeight:'600', marginBottom:12}}>ðŸ“Š Estatísticas</Text>
          ...
          <TouchableOpacity onPress={limparDados} style={{backgroundColor:'rgba(239,68,68,0.12)', paddingVertical:12, borderRadius:30, alignItems:'center', marginBottom:16}}>
            <Text style={{color:'#ef4444', fontSize:14}}>ðŸ—‘ï¸ Limpar todos os dados</Text>
          </TouchableOpacity>

          {/* Sobre */}
          <View style={{backgroundColor:'rgba(255,255,255,0.06)', borderRadius:16, padding:16, marginBottom:16}}>
            <Text style={{color:'#fff', fontSize:16, fontWeight:'600', marginBottom:12}}>â„¹ï¸ Sobre</Text>
